class DailyRoutineController
  ...
  def add
    ...
    if daily_routine.new_record?
      ...
      puts "#{name} has been added to the daily routines."
    end
  end
  ...
  def view(daily_routine_number)
    ...
    if daily_routine
      puts "You have selected the #{daily_routine.name} daily routine."
      workouts_controller = WorkoutsController.new(daily_routine)
      workouts_controller.list
    else
      puts "The daily routine '#{daily_routine_number}' doesn't exist."
    end
  end
